...
async function main() {
  ...
  console.log("Model before:", before?.weights ?? DEFAULT_WEIGHTS);
  ...
  console.log("Feedback count:", await feedbackRepository.count());
  ...
  console.log("Model after:", after);

  if (!after) {
    throw new Error("Model was not saved");
  }

  console.log("Feedback loop OK");
}
